...
rm(list=ls())
source("~/R/NEON-drift-correction/pack/wrap.xml.read.prse.cval.R")
source("~/R/NEON-drift-correction/pack/def.read.cal.xml.neon.str.mtch.R")
...
rsltAsst <- def.read.asst.xml.neon(asstXml = xml)
...
urlApi <- base::paste0(urlBaseApi,'/calibrations?asset-stream-key=',rsltAsst$assetUid[idxInst],':',rsltAsst$strmId[idxInst],
                       '&startdate=',rsltAsst$installDateText[idxInst],'&enddate=','2021-03-01T00:00:00.000Z',collapse='')
